...
const AddToWishlistBtn = ({ product, slug }: AddToWishlistBtnProps) => {
  ...
  const addToWishlistFun = async () => {
    ...
    if (session?.user?.email) {
      ...
      fetch(`https://ngo-server-production.up.railway.app/api/users/email/${session?.user?.email}`, {
        cache: "no-store",
      })
        .then((response) => response.json())
        .then((data) =>
          fetch("https://ngo-server-production.up.railway.app/api/wishlist", {
            method: "POST",
            headers: {
  ...
  const removeFromWishlistFun = async () => {
    if (session?.user?.email) {
      ...
      fetch(`https://ngo-server-production.up.railway.app/api/users/email/${session?.user?.email}`, {
        ...
        .then((data) => {
          return fetch(
            `https://ngo-server-production.up.railway.app/api/wishlist/${data?.id}/${product?.id}`,
            {
              method: "DELETE",
            }
          );
